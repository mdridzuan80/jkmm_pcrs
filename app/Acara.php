...
class Acara extends Eventable
{
    ...
    public function scopeEvents($query)
    {
        return $query->select(
            ...
            DB::raw('tarikh_mula_time'),
            DB::raw('tarikh_tamat_time'),
            ...
            DB::raw('keterangan'),
            DB::raw('kategori'),
            DB::raw('medan_kesalahan'),
            DB::raw('flag_kelulusan'),
    ...
    {    
		...
		//$txtTarikhHingga_format = '2021-08-03 00:00:00';
	 	...
	 	public function scopedetail_acara_individu_ikuttarikh2($query, $profil, $tarikh)
	//public function scopedetail_checkinout_individu($query,$comPegawai='0')
    {
		
	}
	
	 	public function scopedetail_acara_individu_ikuttarikh($query, $tarikh)
	//public function scopedetail_checkinout_individu($query,$comPegawai='0')
    {    
		...
		//echo $tarikh;
			...
			$tahunbulanhari = explode ("-", $tarikh); 
			
			$tahunQ = $tahunbulanhari[0];
			$bulanQ = $tahunbulanhari[1];
			$hariQ = $tahunbulanhari[2];
			
			$tahunbulanhariQ = explode (" ", $hariQ);
			
			$hariQ1 = $tahunbulanhariQ[0]; 
			
			$tarikhQ = $tahunQ.'-'.$bulanQ.'-'.$hariQ1;
			...
			->where('anggota_id', '=', '3')		
			//->where('anggota_id', '=', $profil->xtraAttr->anggota_id)	
			->where('tarikh_mula_date', '<=', $tarikhQ)
			->where('tarikh_tamat_date', '>=', $tarikhQ)
			//->where('tarikh_mula_date', '<=', '2022-04-04')
			//->where('tarikh_tamat_date', '>=', '2022-04-04')
			->orderBy('tarikh_mula_date')
			->get();
