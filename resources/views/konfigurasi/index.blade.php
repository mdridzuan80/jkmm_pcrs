        ...
        <div class="box-body">
            <div class="nav-tabs-custom">
                <ul class="nav nav-tabs">
                    ...
                    @endcan
                    <li><a id="tab_bdr" href="#tab_4">Modul BDR</a></li>
                </ul>
                                                ...
                                                <tr>
                                                    <td>
                                                        <div style="position: relative;">
                                                            ...
                                                            <div id="treeDisplay" style="display:none; position: relative;">
                                            ...
                                            <td style="width: 60%">
                                                <table class="table" style="width: 100%; background-color: transparent;">
                                                    ...
                                                    <tr>
                                                        <td colspan="3">
                                                            <ol>
                                                                <li v-for="puasa of senPuasa" :key="puasa.id">
                        ...
                        <table class="table table-striped table-responsive">
                            <tbody>
                                <tr id="app-bdr">
                                    <div v-cloak>
                                        <td style="width: 40%">
                                            <b>Aktifkan Modul Bekerja Dari Rumah</b>
                                            <p class="help-block">
                                                Mengaktifkan modul bekerja dari rumah mengikut keperluan bahagian atau unit.
                                            </p>
                                        </td>
                                        <td style="width: 60%">
                                            <table class="table" style="width: 100%; background-color: transparent;">
                                                <tr>
                                                    <td colspan="3">
                                                        <div style="position: relative;">
                                                            <input id="departmentDisplayBdr" class="form-control departmentDisplayBdr" type="text" style="background-color: #FFF;" readonly disabled placeholder="Bahagian/ Unit">
                                                            <input id="departmentDisplayIdBdr" name="txtDepartmentId" v-model="bahagian_id" class="form-control departmentDisplayIdBdr" type="hidden" style="background-color: #FFF;" readonly>
                                                            <div id="treeDisplayBdr" style="display:none; position: relative;">
                                                                <div id="departmentsTreeBdr" style="position:absolute; background-color: #FFF; overflow:auto; max-height:200px; border:1px #ddd solid"></div>
                                                            </div>
                                                        </div>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        <input type="text" id="tkhMulaBdr" name="txtMula" class="form-control" placeholder="Tarikh Mula" v-model="tkhMula">
                                                    </td>
                                                    <td>
                                                        <input type="text" id="tkhTamatBdr" name="txtTamat" class="form-control" placeholder="Tarikh Tamat" v-model="tkhTamat">
                                                    </td>
                                                    <td>
                                                        <button class="btn btn-info" @click="updateTarikh">Simpan</button>
                                                    </td>
                                                        ...
                                                        <template v-if="senBdr.length">
                                                            <table class="table table-bordered table-hover">
                                                                <tbody>
                                                                    ...
                                                                    <tr v-for="bdr of senBdr" :key="bdr.id" class="row-shift" data-shiftid="1">
                                                                        <td width="1">1</td>
                                                                        <td>@{{ bdr.bahagian }}</td>
                                                                        <td>@{{ bdr.tkhmula }}</td>
                                                                        <td>@{{ bdr.tkhtamat }}</td>
                                                                        <td>
                                                                            <a title="Hapus Masa BDR" class="btn btn-danger btn-xs" @click="deleteTarikh(bdr.id)">
                                                                                <i class="fa fa-trash-o"></i>
                                                                            </a>
                                                                        </td>
                                                                    </tr>
    ...
    $(function() {
        ...
        $.ajax({
            ...
            success: function(result, textStatus, jqXHR) {
                ...
                $('#departmentDisplayBdr').prop('disabled', false);
        ...
        var appPuasa = new Vue({
                        ...
                        preConfirm: (email) => {
                            return new Promise((resolve, reject) => {
                                $.ajax({
                                    ...
                                    data: {
                                        tkhMula: this.tkhMula,
                                        tkhTamat: this.tkhTamat
                                    },
            ...
            mounted() {
                ...
                $('#tkhPuasaTamat').datepicker({
                    ...
                    this.tkhTamat = $('#tkhPuasaTamat').val()
                });
            },
            created() {
                $.ajax({
        ...
        var appBdr = new Vue({
            ...
            methods: {
                updateTarikh() {
                    swal({
                        title: 'Amaran!',
                        text: 'Anda pasti untuk menambah rekod?',
                        type: 'warning',
                        cancelButtonText: 'Tidak',
                        showCancelButton: true,
                        confirmButtonText: 'Ya!',
                        ...
                        if (result.value) {
                            this.senBdr.push(result.value.data);
                ...
                deleteTarikh(id) {
                    swal({
                        title: 'Amaran!',
                        text: 'Anda pasti untuk menghapuskan rekod ini?',
                        type: 'warning',
                        cancelButtonText: 'Tidak',
                        showCancelButton: true,
                        confirmButtonText: 'Ya!',
                        ...
                        if (result.value) {
                            this.senBdr = this.senBdr.filter(bdr => bdr.id != id);
                        }
                    }).catch(function() {
                        swal({
                            title: 'Ralat!',
            ...
            mounted() {
                $('#tkhMulaBdr').datepicker({
                    format: 'yyyy-mm-dd',
                    todayHighlight: true,
                    autoclose: true
                }).on('changeDate', () => {
                    this.tkhMula = $('#tkhMulaBdr').val()
                })

                $('#tkhTamatBdr').datepicker({
                    format: 'yyyy-mm-dd',
                    todayHighlight: true,
                    autoclose: true
                }).on('changeDate', () => {
                    this.tkhTamat = $('#tkhTamatBdr').val()
                });

                $('#departmentDisplayBdr').on('click', function(e) {
                    e.preventDefault();
                    $('#departmentsTreeBdr').css('width', $(this).parent().actual('width'));
                    $('#departmentsTree').jstree('select_node', $('.departmentDisplayIdBdr').val().toString());
                    $('#treeDisplayBdr').toggle();

                    $(document).click(function(e) {
                        if (!$(e.target).hasClass("departmentDisplayBdr") &&
                            $(e.target).parents("#treeDisplayBdr").length === 0) {
                            $("#treeDisplayBdr").hide();
                        }
                    });
                });

                $('#departmentsTreeBdr').on('select_node.jstree', function(e, data) {
                    var id = data.instance.get_node(data.selected[0]).id;
                    var text = data.instance.get_node(data.selected[0]).text;

                    $('.departmentDisplayBdr').val(text);
                    $('.departmentDisplayIdBdr').val(id);
                    dataBdr.bahagian_id = id;
                    $("#treeDisplayBdr").hide();
                });
            },
            created() {
                $.ajax({
                    url: base_url + "rpc/bdr/",
                    success: response => {
                        this.senBdr = response.data.map((item) => {
                            item.tkhmula = moment(item.tkhmula).format('DD-MM-YYYY');
                            item.tkhtamat = moment(item.tkhtamat).format('DD-MM-YYYY');
                            return item;
                        });
